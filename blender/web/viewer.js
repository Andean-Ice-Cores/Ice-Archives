...
(function () {
  ...
  const labels = new THREE.CSS2DRenderer(); labels.setSize(innerWidth, innerHeight);
  labels.domElement.style.position = 'fixed'; labels.domElement.style.inset = '0'; labels.domElement.style.pointerEvents = 'none';
  ...
  function tick() {
    ...
    renderer.render(scene, camera); labels.render(scene, camera); requestAnimationFrame(tick);
  }
  ...
  document.getElementById('shot').onclick = () => { const a = document.createElement('a'); a.download = `andes_enso_${state.period}_${state.show}${state.period === 'run' ? '_' + state.year : ''}.png`; a.href = renderer.domElement.toDataURL('image/png'); a.click(); };
  ...
  function renderCard() {
    ...
    document.getElementById('card').innerHTML = `
      ...
      <div class="muted" style="margin-top:8px">Full statistics, composites and pre-1950 reconstructions: <a href="../../app/index.html">2-D analysis app</a> · <a href="../out/running.mp4">Blender animation</a></div>`;
  }
})();
